...
def merge_query_parameters(query_parameter_dictionaries: List[dict]) -> dict:
    return {parameter: value for parameter_dict in query_parameter_dictionaries for parameter, value in parameter_dict.items()}


def create_query_parameters_from_preceding_and_succeeding_routing_edge(
    preceding_routing_edge: AnyRoutingEdgeIncomingOrOutgoing, succeeding_routing_edge: AnyRoutingEdgeIncomingOrOutgoing
) -> dict:
...
def do_get_routing_edges_request(
    communication_layer: CommunicationLayer, routing_point: RoutingPoint, routing_edge_type: Optional[RoutingEdgeType]
...
def do_get_any_parameter(communication_layer: CommunicationLayer, key: str) -> Optional[Union[bool, dict, int, list, str]]:
...
def do_get_any_link(
    ...
    query_parameters = merge_query_parameters([convert_any_object(time_window), dict(linkType=link_type, nodeFilter=node_filter)])
