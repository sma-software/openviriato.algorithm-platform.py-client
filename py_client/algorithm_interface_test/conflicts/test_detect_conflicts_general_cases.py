...
class TestDetectConflicts(unittest.TestCase):
    ...
    def test_unknown_conflict(self, mocked_get_obj):
        ...
        self.assertEqual(
            conversion_error.exception.message, "Could not parse Enum unexisting, invalid enum format for expected class Enum <enum 'ConflictType'>"
        )
...
class TestDetectConflictsWithNonMandatoryFilters(unittest.TestCase):
    ...
    def test_detect_conflicts_with_non_mandatory_filters_session(self, mocked_get_obj):
        ...
        arguments = (
            ConflictDetectionArguments(train_ids=train_ids)
        ...
        self.assertEqual(session_obj._DetectConflictsWithNonMandatoryFiltersMockSession__last_request, get_api_url() + "/services/trains:detect-conflicts")

        self.assertDictEqual(
            session_obj._DetectConflictsWithNonMandatoryFiltersMockSession__last_body,
            {
                "trainIds": [6745, 6750],
                "filters": {
                    "location": {
        ...
        self.assertDictEqual(
            session_obj._DetectConflictsWithNonMandatoryFiltersMockSession__last_params,
            {"trainId": 1230, "fromTime": "2022-01-23T23:45:00", "toTime": "2022-01-24T00:15:00"},
        )
