...
class TestUpdateMovementType(unittest.TestCase):
    ...
    def test_update_movement_type(self, mocked_get_obj):
        ...
        self.assertEqual(
            session_obj._PutUpdateMovementTypeMockSession__last_request, get_api_url() + "/trains/{}/train-path-nodes:update-movement-type".format(train_id)
        )

        self.assertDictEqual(
            session_obj._PutUpdateMovementTypeMockSession__last_body,
            dict(movementTypeId=movement_type_id, fromTrainPathNodeId=from_train_path_node_id, toTrainPathNodeId=to_train_path_node_id),
        )
