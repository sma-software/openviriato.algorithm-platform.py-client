...
class TestCalculateRunningTimesPenalties(unittest.TestCase):
    ...
    def test_calculate_running_time_penalties(self, mocked_get_obj):
        ...
        self.assertEqual(
            session_obj._CalculateRunningTimePenaltiesMockSession__last_request,
            get_api_url() + "/services/trains/{}:calculate-running-time-penalties".format(train_id),
        )

        self.assertDictEqual(session_obj._CalculateRunningTimePenaltiesMockSession__last_body, dict())
    ...
    def test_calculate_running_time_penalties_response(self, mocked_get_obj):
        ...
        self.assertIsInstance(list_of_running_time_penalties[0], RunningTimePenaltyOnTrainPath)
        self.assertEqual(list_of_running_time_penalties[0].possession_id, 6745)
        self.assertEqual(list_of_running_time_penalties[0].from_train_path_node_id, 6738)
        self.assertEqual(list_of_running_time_penalties[0].to_train_path_node_id, 6739)
        self.assertEqual(list_of_running_time_penalties[0].required_running_time_penalty, timedelta(minutes=2))
