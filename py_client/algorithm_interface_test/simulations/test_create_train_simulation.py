...
class TestCreateTrainSimulation(unittest.TestCase):
    ...
    def test_create_train_simulation(self, mocked_get_obj):
        requested_time_window = py_client.aidm.aidm_time_window_classes.TimeWindow(
            from_time=datetime.datetime(2003, 5, 5, 7, 0),
            to_time=datetime.datetime(2003, 5, 5, 9, 0))

        self.interface_to_viriato.create_train_simulation(time_window=requested_time_window)

        session_obj = self.interface_to_viriato._AlgorithmInterface__communication_layer.currentSession
        self.assertEqual(session_obj._PutCreateTrainSimulationMockSession__last_request,
                         get_api_url() + "/services/trains/simulations")
        self.assertDictEqual(session_obj._PutCreateTrainSimulationMockSession__last_body,
                             dict(fromTime='2003-05-05T07:00:00', toTime='2003-05-05T09:00:00'))
