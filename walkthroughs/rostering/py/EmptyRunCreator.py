...
class EmptyRunCreator:
    ...
    def __get_empty_run_movement_type(self):
        ...
        if len(empty_run_movement_types) == 0:
            self.__algorithm_interface.notify_user(
                "Warning", "Did not find an empty run movement type. Please add one to the Database. Creating a productive run."
            )
            return None
        else:
    ...
    def create_empty_run_common_activity(
        self,
        first_common_activity_succeeding_train: CommonActivity,
        last_common_activity_preceding_train: CommonActivity,
        template_for_empty_run: AlgorithmTrain,
    ) -> CommonActivity:
    ...
    def __create_empty_run_train(
        self, from_common_activity: CommonActivity, to_common_activity: CommonActivity, template_for_empty_run: AlgorithmTrain
    ) -> AlgorithmTrain:
    ...
    def __update_times_on_empty_run(
        self, empty_run_train: AlgorithmTrain, common_activity_before: CommonActivity, common_activity_after: CommonActivity
    ) -> AlgorithmTrain:
        ...
        train_with_updated_times = self.__algorithm_interface.update_train_trajectory_stop_times(
            empty_run_train.id, UpdateStopTimesTrainPathNode(empty_run_train.train_path_nodes[0].id, departure_time, departure_time, timedelta(0))
        )
