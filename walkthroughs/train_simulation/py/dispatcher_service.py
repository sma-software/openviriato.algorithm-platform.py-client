...
class DispatcherService:
    ...
    def event_of_preceding_train_is_departure_at_given_node(
        simulation_event: AlgorithmTrainSimulationEvent, preceding_tpn_at_given_node: AlgorithmTrainPathNode
    ) -> bool:

        if simulation_event.type is not AlgorithmTrainSimulationEventType.departure:
            return False

        return simulation_event.algorithm_train_path_node_id == preceding_tpn_at_given_node.id
    ...
    def _need_to_postpone_preceding_tpn(
        ...
        if not DispatcherService.event_of_preceding_train_is_departure_at_given_node(any_simulation_event, preceding_tpn_at_given_node):
            ...
            return departure_event_of_preceding_train_at.forecast_time <= succeeding_tpn_at_given_node.departure_time

    @staticmethod
    def _calculate_time_to_postpone_to(simulation_event: AlgorithmTrainSimulationEvent, succeeding_tpn_at_given_node: AlgorithmTrainPathNode) -> timedelta:
        return succeeding_tpn_at_given_node.departure_time - simulation_event.forecast_time + timedelta(minutes=1)
